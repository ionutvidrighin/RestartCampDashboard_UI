...
import { formDataCourseDateInFuture } from './helperMethodsCoursePresence';
...
const CourseDateIsInFuture = ({ hasEditPermission, FormikProps, localStyles }) => {
  const { values, handleChange, errors } = FormikProps

  const courseDateInFuture = formDataCourseDateInFuture(values, errors)
  const [addLinkOnWords, setAddLinkOnWords] = useState(courseDateInFuture)
  ...
  return (
    <div className='section-content mb-2'>
      <p className='section-title m-0'>Curs/sesiune va avea loc in viitor</p>
      { courseDateInFuture.map(element => {
        const objectKeyLocation = 'courseDateIsInFuture'
...
export default CourseDateIsInFuture